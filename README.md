...
### Usage
* Copy the target pdf file to the root directory, replacing the (example_document.pdf)[example_document.pdf]. Only one pdf file should be in the root directory at any time, unless its filename ends with *_signed.pdf* (in which case it is the output document). Needless to say that input documents must not have filenames *\*_signed.pdf* 
